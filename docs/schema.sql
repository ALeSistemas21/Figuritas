...
CREATE TABLE IF NOT EXISTS public.localidades (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL,
    CONSTRAINT unique_localidad_provincia UNIQUE (provincia_id, nombre)
);

-- Indexar por provincia para acelerar las búsquedas y ordenamiento
CREATE INDEX IF NOT EXISTS idx_localidades_provincia ON public.localidades(provincia_id);
...
-- 4. Tabla de Perfiles de Coleccionistas
...
CREATE INDEX IF NOT EXISTS idx_perfiles_cercania ON public.perfiles(provincia_id, localidad_id, escuela_id);
...
-- Para simplificar la importación desde el script de Python, permitiremos inserts/selects públicos en provincias, localidades y escuelas
